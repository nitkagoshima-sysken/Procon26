...
int main()
{
    ...
    for (int y = -STONE_SIZE + 1; y < BOARD_SIZE; y++)
    {
        for (int x = -STONE_SIZE + 1; x < BOARD_SIZE; x++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                Stone *stonetmp = getTouchingStone(obstacleBoard, rotate(&prob->stones[0], j), x, y, true);
                int touchNum = countBitOfStone(stonetmp); 
                delete stonetmp;
                if (!isPut[0] && canPlace(obstacleBoard, putBoard, rotate(&prob->stones[0], j), x, y, true))
                {
                    Board *tmp = cloneBoard(obstacleBoard);
                    tmp = placeStone(obstacleBoard, rotate(&prob->stones[0], j), x, y); 
                    int countNum, cnt;
            ...
            for (int j = 0; j < 4; j++)
            {
                Stone *stonetmp = getTouchingStone(obstacleBoard, rotate(flip(&prob->stones[0]), j), x, y, true);
                int touchNum = countBitOfStone(stonetmp);                    
                delete stonetmp;
                if (!isPut[0] && canPlace(obstacleBoard, putBoard, rotate(flip(&prob->stones[0]), j), x, y, true))
                {
                    Board *tmp = cloneBoard(obstacleBoard);
                    tmp = placeStone(obstacleBoard, rotate(flip(&prob->stones[0]), j), x, y);
                    int countNum, cnt;
                    getGroupsCountBoard(tmp, 0, &countNum, &cnt); 
                    if (countNum < counting && touching < touchNum)
                    {
